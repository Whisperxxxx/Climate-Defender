...
public class MagicUI : MonoBehaviour
{
    ...
    void Update()
    {
        UpdateMagicIcon(PlayerController.Instance.selectedMagicIndex);
    }

    // update the UI to show the correct magic icon
    public void UpdateMagicIcon(int magicIndex)
    {
        if (magicIndex >= 0 && magicIndex < PlayerController.Instance.magicPrefabs.Count) // update the icon only when the magic exists
        {
            SpriteRenderer renderer = PlayerController.Instance.magicPrefabs[magicIndex].GetComponent<SpriteRenderer>();
            if (renderer != null)
            {
    ...
    {
        StartCoroutine(CountDownRoutine(PlayerController.Instance.GetMagicCoolDown()));
